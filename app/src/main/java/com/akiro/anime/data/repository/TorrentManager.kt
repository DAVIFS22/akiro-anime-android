...
class TorrentManager(private val context: Context) {
    ...
    suspend fun streamFromMagnet(
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                val localServer = LocalTorrentHttpServer(video)
                localServer.start()
                server = localServer
     ...
     */
    private class LocalTorrentHttpServer(private val file: File) {
        ...
        private fun serve(client: Socket) {
            client.use { socket ->
                ...
                // HEAD is useful to ExoPlayer for probing duration/length.
                val length = waitForFile()
                if (length <= 0L) {
        ...
        /**
         * libtorrent allocates the file to its final size. For that reason, checking
         * file.length() alone cannot tell whether a byte has arrived yet. Sequential
         * mode makes this loop useful for startup, while libtorrent continues filling
