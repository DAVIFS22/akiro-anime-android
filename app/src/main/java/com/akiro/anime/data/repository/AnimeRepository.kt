...
fun posterUrl(path: String?, size: String = "w500"): String =
    if (path.isNullOrEmpty()) "https://image.tmdb.org/t/p/w500"
    else "https://image.tmdb.org/t/p/$size$path"

fun backdropUrl(path: String?, size: String = "w1280"): String =
    if (path.isNullOrEmpty()) "https://image.tmdb.org/t/p/w1280"
    else "https://image.tmdb.org/t/p/$size$path"
...
fun com.akiro.anime.network.AnimeDto.toDomain(): Anime = Anime(
    id = id, tmdbId = tmdb_id, malId = mal_id, imdbId = imdb_id, kitsuId = kitsu_id,
    title = title, romajiTitle = romaji_title, nativeTitle = native_title, synopsis = synopsis,
    poster = poster, banner = banner, backdrop = backdrop, genres = genres, year = year,
    status = runCatching { AnimeStatus.valueOf(status) }.getOrDefault(AnimeStatus.EM_EXIBICAO),
    type = runCatching { AnimeType.valueOf(type) }.getOrDefault(AnimeType.TV), rating = rating,
    ratingScoreCount = rating_score_count, ageRating = age_rating, studios = studios,
    totalEpisodes = total_episodes, seasons = seasons.map { Season(it.season_number, it.name, it.episode_count, year = it.year) }
)

private fun com.akiro.anime.network.EpisodeDto.toDomain() = com.akiro.anime.data.model.Episode(
    id = id, number = number, seasonNumber = 0, title = title, thumbnail = thumbnail,
    duration = duration, description = description, airDate = air_date
)
...
class AnimeRepository {
    ...
    suspend fun getTrending(): List<Anime> {
        if (AkiroApiClient.isConfigured) return AkiroApiClient.api.trending(30).items.map { it.toDomain() }
    ...
    suspend fun getPopular(): List<Anime> {
        if (AkiroApiClient.isConfigured) return AkiroApiClient.api.popular(30).items.map { it.toDomain() }
        return api.getPopularAnime(apiKey).results.map { it.toAnime() }
    }

    suspend fun search(query: String): List<Anime> {
        if (AkiroApiClient.isConfigured) return AkiroApiClient.api.search(query).items.map { it.toDomain() }
        return api.searchAnime(apiKey, query).results
    ...
    suspend fun getDetails(tmdbId: Int): Anime {
        if (AkiroApiClient.isConfigured) {
            return AkiroApiClient.api.getAnime(tmdbId).toDomain()
        }
    ...
    suspend fun getSeasonEpisodes(tmdbId: Int, seasonNumber: Int): List<com.akiro.anime.data.model.Episode> {
        if (AkiroApiClient.isConfigured) {
            return AkiroApiClient.api.getEpisodes(tmdbId, seasonNumber).items.map { it.toDomain().copy(seasonNumber = seasonNumber) }
        }
