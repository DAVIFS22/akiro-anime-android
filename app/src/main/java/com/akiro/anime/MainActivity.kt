...
fun AkiroApp() {
    ...
    Scaffold(bottomBar = { if (showBottom) AkiroBottomBar(navController) }) { padding ->
        NavHost(navController, startDestination = "home", modifier = Modifier.padding(padding)) {
            composable("home") { HomeScreen({ navController.navigate("detail/$it") }, onSearch = { navController.navigate("search") }) }
            composable("search") { SearchScreen { navController.navigate("detail/$it") } }
            composable("favorites") { FavoritesScreen { navController.navigate("detail/$it") } }
